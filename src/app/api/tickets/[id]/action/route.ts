...
export async function POST(
  ...
  try {
    ...
    // Special case for TIMEOUT_ESCALATE (does its own dispatching without holding a transaction lock)
    if (action === "TIMEOUT_ESCALATE") {
      ...
      if (ticket.status === "CREATED" && ticket.technicianId) {
        ...
        return NextResponse.json({
          success: true,
          message: "ประมวลผล timeout escalation สำเร็จ",
          ticket: refreshed,
        });
      }

      return NextResponse.json({ success: true, message: "ไม่มีการเปลี่ยนแปลง" });
    }
    ...
    // Clear timeout timer if accepted
    if (action === "ACCEPT" && result?.ticket?.id) {
      clearTicketTimer(result.ticket.id);
    }
